...
;(function(factory) {
    ...
    EvoCalendar = (function() {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.initials = {
                ...
                dates: {
                    en: {
                        ...
                        noEventForToday: "No event or vacation day to work",
                        noEventForThisDay: "No event or vacation day to work",
						enjoyWeekend: "It is the weekend enjoy in it ",
						deletee: "Delete",
                    },
					ar: {
                        days: ["الاحد", "الاثنين", "الثلاثاء", "الاربعاء", "الخميس", "الجمعة", "السبت"],
                        daysShort: ["احد", "اثنين", "ثلاثاء", "اربعاء", "خميس", "جمعة", "سبت"],
                        daysMin: ["حد", "ثنين", "ثلاث", "اربع", "خميس", "جمعة", "سبت"],
                        months: ["يناير", "فبراير", "مارس", "ابريل", "مايو", "يونيو", "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"],
                        monthsShort: ["ينا", "فبر", "مار", "ابر", "ماي", "يون", "يول", "أغس", "سبت", "أكت", "نوف", "ديس"],
                        noEventForToday: "لا يوجد احداث او اجازات يوم عمل رسمي",
                        noEventForThisDay: "لا يوجد احداث او اجازات يوم عمل رسمي",
						enjoyWeekend: "عطلة نهاية الاسبوع استمتع باجازتك",
						deletee: "حذف",
                    },
                    es: {
			...
			_.initials.weekends = [];
			$.each( _.options.weekends, function( key, value ) 
			{
				_.initials.weekends[key] = _.initials.dates[_.options.language].daysShort[value];
			});
			//console.log($.type(_.initials.weekends));
    ...
    EvoCalendar.prototype.buildTheBones = function() {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup += '<div class="calendar-inner">'+
                            '<table class="calendar-table">'+
                                ...
                                for(var i = 0; i < _.$label.days.length; i++ ){
									...
									//me
									if(jQuery.inArray(_.$label.days[i], _.initials.weekends) !== -1)
									{
                                        headerClass += ' --weekend';
                                    }
            ...
            markup += '<div class="calendar-events">'+
							...
							'<div class="forms_buttons pull-right"></div>'+
                        '</div>';
    ...
    EvoCalendar.prototype.buildEventList = function() {
		...
		//me  buttons (add,edit,delete)
		var buttons = _.$elements.eventEl.find('.forms_buttons');
        ...
        if(!hasEventToday) {
			...
			//me
			var nos = new Date(_.$active.date);
			//console.log(_.$active.date);
			//console.log(nos);
			if(jQuery.inArray(_.initials.dates[_.options.language].daysShort[nos.getDay()], _.initials.weekends)  !== -1	)
			{
                markup += `<p>${_.initials.dates[_.options.language].enjoyWeekend}</p>`;
            } else {
                markup += `<p>${_.initials.dates[_.options.language].noEventForThisDay}</p>`;
            }
            markup += '</div>';
			var addhtml= '<button id="add_vacation" data-toggle="modal" data-target="#modal-event" class="forms_buttons-action">'+add+' '+vacation+' <i class="fa fa-plus-circle"></i></button>';
			buttons.html(addhtml);
        }
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
		...
		if(event_data.type == "holiday")
		{
			markup = '<div class="event-container" role="button" data-toggle="modal" data-target="#modal-event'+(event_data.id)+'" data-event-index="'+(event_data.id)+'">';
		}
		else //worked day
		{
			markup = '<div class="event-container work" role="button"  data-event-index="'+(event_data.id)+'" data-url="'+(event_data.url)+'" >';
		}
		...
		if(event_data.type == "holiday")
		{
			var addhtml= '<button class="btn btn-danger btn-xs" name="delete" data-section="#page_section" data-contanier="#page_container" data-url="" data-event="'+(event_data.id)+'"><span class="glyphicon glyphicon-trash"></span> '+`${_.initials.dates[_.options.language].deletee}`+'</button>';
			buttons.html(addhtml);
		}
		else{var addhtml= ''; buttons.html(addhtml);}
                    ...
                    for (var i = 0; i < 9; i++) { // this loop is for is weeks (rows)
                        for (var j = 0; j < _.$label.days.length; j++) { // this loop is for weekdays (cells)
                            if (day <= _.monthLength && (i > 0 || j >= _.startingDay)) {
								...
								if(jQuery.inArray(_.$label.days[j], _.initials.weekends) !== -1)
								{
