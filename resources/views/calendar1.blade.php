					...
					<div id="last_section">
           				<div class="" id="last_container">
							...
							<div class="" id="change_vacations">
								...
								<div class="modal" id="modal-event{{$vacation->id}}"   >
								    <div class="modal-dialog">
									    <div class="modal-content">
									        <div class="modal-header">
									        	<button type="button" class="close" data-dismiss="modal" ><span class="glyphicon glyphicon-remove" ></span></button>
									      	...
									      	<form class="forms_form validate-form" id="vacation_form{{$vacation->id}}"  data-section="#page_section" data-contanier="#page_container" data-url="{{route('add_vacation',$vacation->id)}}" data-page="">
										    @csrf
									      		<div class="modal-body">
										          ...
										          <fieldset class="forms_fieldset">
										           ...
										           <div class="forms_field validate-input @error('date') has-invalid alert-validate @enderror" data-validate="@error('date'){{ $message }} @else{{ __('loginRegister.valid_date') }} @enderror">
										              <span><i class="fa fa-calendar-times-o"></i> {{ __('loginRegister.date') }}:</span>
										              <input type="date" placeholder="{{ __('loginRegister.place_date') }}" class="forms_field-input bigger" name="date" value="{{$vacation->date}}" required readonly="true" />
										            </div>
										           ...
										           <div class="forms_field validate-input bigger @error('slct') has-invalid alert-validate @enderror" data-validate="@error('slct'){{ $message }} @else{{ __('loginRegister.choose_vacation') }} @enderror" >
										              <div class="select">
										                ...
										                @foreach($daystatuses as $daystatus)
										                    @if(app()->getLocale()=="ar" )
										                      <option @if($vacation->day_status_id == $daystatus->id) selected @endif value="{{$daystatus->id}}">{{$daystatus->name_ar}}</option>
										                    @else
										                      <option @if($vacation->day_status_id == $daystatus->id) selected @endif value="{{$daystatus->id}}">{{$daystatus->name_en}}</option>
										                    @endif
									                  	@endforeach
							...
							<div id="vacations" style="display: none;" data-vacations="{{json_encode($vacations_events)}}"></div>
	...
	<script src="{{ asset('js/evo-calendar.js') }}"></script>
        ...
        $(document).ready(function() 
        {
			...
			$(document).on( "click",".icon-button[data-year-val='prev'] , .icon-button[data-year-val='next']", function(event)
			{
				...
				if(response.hasOwnProperty("success") )
				{
					console.log(response,response.vacations_events);
			...
			{
				//var vacations = {!! json_encode($vacations_events) !!};
